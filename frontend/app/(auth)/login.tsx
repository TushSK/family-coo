...
export default function LoginScreen() {
  ...
  async function handleGoogle() {
    setLoading(true);
    try {
      // TODO: replace with real expo-auth-session Google OAuth
      await new Promise(r => setTimeout(r, 800));
      // markLogin writes to AsyncStorage AND updates context state atomically
      await markLogin(USER_ID);
      // AuthGate in _layout.tsx will detect auth="pin" and redirect automatically
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
...
const st = StyleSheet.create({
  ...
  googleBtn:  { width:"100%", backgroundColor:C.bgCard, borderRadius:R.lg, borderWidth:0.5, borderColor:C.border2, flexDirection:"row", alignItems:"center", justifyContent:"center", paddingVertical:15, gap:12, marginBottom:20 },
